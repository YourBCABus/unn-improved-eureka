...
pub async fn unwrap_connection(connection_result: Result<PgPool, sqlx::Error>) -> PgPool {
    match connection_result {
        Ok(client) => client,
        Err(e) => {
            logging::report!("Failed to connect to eureka db": { "err": e.to_string() });
            logging::error!("Failed to connect to eureka db: {e}");
            logging::debug!("Eureka db error: {e:#?}");
...
#[doc(hidden)]
pub use server::AppState as __AppState;
