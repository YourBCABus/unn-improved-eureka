        ...
        match cmd {
            MetricsCommand::Read { range, step } => {
                ...
                if let Err(e) = responder.send(output) {
                    logging::error!("Failed to send response to metrics command: {:?}", e);
                    logging::info!("Timeout will likely trigger...");
                }
            }
    ...
    async fn send(&self, command: MetricsCommand, timeout: Duration) -> Result<SparseMetricsView, SendError<MetricsCommand>> {
        ...
        tokio::select! {
            res = reciever => {
                match res {
                    Ok(view) => {
                        logging::debug!("Successfully recieved metrics view: {view:?}");
                        Ok(*view)
                    },
                    Err(e) => {
                        logging::debug!("Failed to recieve metrics view - recieve failed: {e:?}");
                        Err(SendError(command))
                    },
                }
            },
            _ = tokio::time::sleep(timeout) => {
                logging::debug!("Failed to recieve metrics view - {:.4} second timeout expired", timeout.as_secs_f64());
                Err(SendError(command))
            },
        }
    }
