...
impl From<SqlTeacherInfo> for Option<Teacher> {
    fn from(sql: SqlTeacherInfo) -> Self {
        ...
        let middles = name_middle_display.into_iter()
            .zip(name_middle_texts)
            .map(|(vis, name)| crate::types::MiddleName {
                vis,
                name,
            })
            .collect();
...
pub async fn create_teacher(ctx: &mut Ctx, school_id: Uuid, input: Teacher) -> Result<Teacher, sqlx::Error> {
    ...
    let middle_texts: Vec<_> = input.get_name()
        .all_middles()
        .map(|middle| middle.name.clone())
        .collect();
    let middle_display: Vec<_> = input.get_name()
        .all_middles()
        .map(|middle| middle.vis)
        .collect();
...
pub async fn update_teacher_name(ctx: &mut Ctx, id: Uuid, name: TeacherName) -> sqlx::Result<Teacher> {
    ...
    let middle_texts: Vec<_> = name
        .all_middles()
        .map(|middle| middle.name.clone())
        .collect();
    let middle_display: Vec<_> = name
        .all_middles()
        .map(|middle| middle.vis)
        .collect();
